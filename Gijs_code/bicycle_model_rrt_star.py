...
#        p.loadURDF("cube.urdf", [x, y, z], globalScaling=1.0)
    ...
    for x in range(-2, 9, 1):
        pos = [x, 1.5, 0.5]
        p.loadURDF("cube.urdf", pos, globalScaling=1.0)
...
def rrt(start, goal, obstacles):
    ...
    for _ in range(NUM_INITIAL_BRANCHES):
        steering_angle = random.uniform(-np.pi, np.pi)
        ...
        if is_collision_free(initial_branch, obstacles):
            ...
            costs[tuple(initial_branch)] = costs[tuple(start)] + np.linalg.norm(np.array(start[:2]) - np.array(initial_branch[:2]))
    ...
    for _ in range(MAX_NODES):
        ...
        nearest = min(nodes, key=lambda node: np.linalg.norm(np.array(node[:2]) - np.array(rand_state[:2])))
        ...
        # Simulate the motion using the bicycle model
        #steering_angle = random.uniform(-MAX_STEERING_ANGLE, MAX_STEERING_ANGLE)
        #new_state = bicycle_step(nearest, MAX_VELOCITY, steering_angle)

        # Check for collisions
        if is_collision_free(new_state, obstacles):
            new_cost = costs[tuple(nearest)] + np.linalg.norm(np.array(nearest[:2]) - np.array(new_state[:2]))
            ...
            for near_node in near_nodes:
                potential_cost = new_cost + np.linalg.norm(np.array(new_state[:2]) - np.array(near_node[:2]))
                if potential_cost < costs[tuple(near_node)] and is_collision_free(near_node, obstacles):
                    ...
                    # Debug visualization for rewiring
                    p.addUserDebugLine(
            ...
            # Debug visualization in PyBullet
            p.addUserDebugLine(
            ...
            #p.addUserDebugSphere([new_state[0], new_state[1], GOAL_THRESHOLD], radius=3.0, lineColorRGB=[1, 0.4, 0.7], fillColorRGB=[1, 0.4, 0.7])
...
def visualize_path(path, obstacles, start, goal):
    ...
    for obs_x, obs_y, _ in obstacles:
        square = plt.Rectangle((obs_x - 1.5, obs_y - 1.5), 1.5, 1.5, color='black', fill=True)  # Adjusted size to match PyBullet scaling
    ...
    plt.scatter(start[0], start[1], c='green', s=200, label="Start", marker="o")  # Start as a green circle
    plt.scatter(goal[0], goal[1], c='purple', s=200, label="Goal", marker="X")   # Goal as a purple X

    # Configure plot
    plt.xlim(-15, 15)  # Add a larger border
    plt.ylim(-15, 15)
    plt.legend()
    plt.grid(True)
