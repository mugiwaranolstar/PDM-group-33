...
def do_simulation_with_obstacles(cx, cy, cyaw, ck, sp, dl, initial_state):
    ...
    while time_ <= MAX_TIME:
        ...
        if show_animation:
            plt.cla()
            plt.plot(cx, cy, '-r', label='Smoothed RRT* Path')
            # obstacles
            ...
            plt.plot(traj_x, traj_y, '-g', label='Collision-free MPC Path')

            plt.plot(cx[0], cy[0], 'go', markersize=8, label='Start Point')  # Start
            plt.plot(cx[-1], cy[-1], 'r*', markersize=12, label='End Point')  # End
...
def main():
    ...
    if show_animation:
        ...
        for obs in obstacles_for_display:
            circle = plt.Circle((obs[0], obs[1]), obs[2], color='b', alpha=0.3)
            plt.gca().add_artist(circle)
        plt.plot(x, y, '-g', label='Collision-free Path Generated by MPC')
        plt.axis('equal')
        plt.legend()
